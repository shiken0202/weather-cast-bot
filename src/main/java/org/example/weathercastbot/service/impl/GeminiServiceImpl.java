...
public class GeminiServiceImpl implements GeminiService {
    ...
    public String generateWeatherResponse(String userMessage, String targetLocation, WeatherInfoDto weatherContext,
        ...
        String prompt = buildPrompt(userMessage, targetLocation, weatherContext, realTimeContext, town3hContext, weeklyContext);
    ...
    public String generateMultiWeatherResponse(String userMessage, String combinedContext) {
        ...
        prompt.append("【語言與格式絕對指令】：你的所有回覆必須直接且『僅能』使用繁體中文，嚴禁使用任何其他語言（包括英文）。【警告】：絕對不能輸出你的思考過程 (Chain of Thought)、內部草稿、分析筆記或任何大綱！請直接給出您最終要對用戶說的話。\n\n");
        String nowStr = java.time.LocalDateTime.now(java.time.ZoneId.of("Asia/Taipei")).format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd EEEE HH:mm:ss"));
        prompt.append("【當前系統標準時間】：").append(nowStr).append(" (請以此時間為基準來判斷「今天」、「明天」與「後天」的預報對應時間)。\n");
        prompt.append("【時間定義絕對指令】：氣象資料中的「06:00~18:00」屬於【白天(Daytime)】，絕對不可以稱為晚上！「18:00~06:00」才屬於【晚上/今晚/夜間(Night)】。當用戶詢問「今晚」時，請務必只讀取 18:00 之後的資料，嚴禁把 06:00~18:00 的狀況回答給今晚！\n\n");
    ...
    /**
     * Strips any characters from non-CJK/non-ASCII Unicode blocks that the LLM might hallucinate
     * (e.g., Devanagari like Hindi 'तीनों', Arabic, Thai).
     */
    private String sanitizeResponse(String text) {
        if (text == null) return "";
        
        // Fix Gemini's specific Hindi numeral hallucinations to keep sentences grammatical
    ...
    private String buildPrompt(String userMessage, String targetLocation, WeatherInfoDto daily, String rtc, String town3h, String weekly) {
        StringBuilder prompt = new StringBuilder();
        prompt.append("【語言與格式絕對指令】：你的所有回覆必須直接且『僅能』使用繁體中文，嚴禁使用任何其他語言（包括英文）。【警告】：絕對不能輸出你的思考過程 (Chain of Thought)、內部草稿、分析筆記或任何大綱！請直接給出您最終要對用戶說的話。\n\n");
        String nowStr = java.time.LocalDateTime.now(java.time.ZoneId.of("Asia/Taipei")).format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd EEEE HH:mm:ss"));
        prompt.append("【當前系統標準時間】：").append(nowStr).append(" (請以此時間為基準來判斷「今天」、「明天」與「後天」的預報對應時間)。\n");
        ...
        if (daily != null) {
            prompt.append("【近期12小時概況】：大約 ").append(daily.getMinTemp()).append("~").append(daily.getMaxTemp())
                  .append("度，").append(daily.getDescription()).append("，降雨機率 ").append(daily.getRainProbability()).append("（請注意：這是近期的狀況。若用戶詢問明天或其他時間的預報，請務必去讀取下方的詳細預報JSON並給出具體明天的機率，切勿張冠李戴）。\n");
        }
        if (rtc != null) {
        ...
        prompt.append("請直接以自然對話回覆使用者的問題（不要輸出 Markdown 程式碼區塊，只輸出你的最終回答，嚴禁夾帶推理過程或是英文思考段落）。如果你在資料中看到類似 3 小時 JSON 預報，請自行將連續且變動不大的時段「整併」成一個範圍告訴用戶（例如：「今天 06:00 至 18:00 降雨機率皆維持在 70% 左右」），嚴禁像流水帳般把每一個時間點都講一次！時間請一律使用標準數位格式如「06:00」。");
