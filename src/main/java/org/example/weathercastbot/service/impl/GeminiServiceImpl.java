...
public class GeminiServiceImpl implements GeminiService {
    ...
    public String rewriteWarningDescription(String locationName, String originalDescription) {
        ...
        try {
            String promptStr = String.format("以下是中央氣象署發布的最新特報原文：\n" +
                            "「%s」\n\n" +
                            "這段文字通常會統稱某些地理範圍（例如「嘉義以北」、「各地山區」等），導致特定縣市的居民不確定自己是否包含在內。\n" +
                            "現在系統已經確認【%s】就在這次的特報影響範圍名單內。\n" +
                            "請你將這段氣象署的說明文字「微調改寫」，讓它讀起來是專門為【%s】的居民播報的。\n\n" +
                            "【嚴格禁止事項與規則】（違反將導致嚴重後果）：\n" +
                            "1. **絕對不可無中生有**：嚴禁自行發明原文中沒有的天氣成因（例如：原文沒寫滯留低壓，你就不准寫）、嚴禁捏造具體的累積雨量數字！\n" +
                            "2. **保留關鍵地理特徵**：如果原文特別強調「山區」、「沿海」、「低窪地區」，你在改寫時**必須原封不動保留這些詞彙**。例如原文寫「各地山區」，你必須改寫成「【%s】的山區」或保留「山區」字眼，絕對不可直接刪除「山區」而只講【%s】！\n" +
                            "3. **保留所有天氣現象**：必須保留原文提到的所有降雨型態及影響現象（如雷擊、強陣風、坍方、落石、溪水暴漲、積水等），不可隨意刪減。\n" +
                            "4. **語氣專業客觀**：請保持氣象播報員的精煉語氣，句子要順暢，長度不可超過原文太多。\n" +
                            "5. **格式要求**：只輸出最終改寫的結果，絕對不包含任何其他的對話、前言或問候語。",
                    originalDescription, locationName, locationName, locationName, locationName);
